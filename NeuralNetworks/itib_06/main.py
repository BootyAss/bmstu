...
class NN:
    def __init__(self, file_liceums, file_districts):
        self.liceums = pd.read_excel(
            file_liceums+'.xlsx', header=0, engine='openpyxl').values.tolist()
        self.districts = pd.read_excel(
            file_districts+'.xlsx', header=0, engine='openpyxl').drop(columns=['Unnamed: 0']).values.tolist()
